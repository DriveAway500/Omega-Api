...
async def test_single_tag_insertion():
    """Testa inserção individual de tag/sha256."""
    sha = generate_sha256(1)
    await add_tag_sha256("last_modified", sha)

    results = await get_all_by_tag("last_modified")
    assert len(results) == 1
    assert results[0] == ("last_modified", sha)
...
async def test_deduplication():
    ...
    await add_tag_sha256("duplicate_test", sha)
    await add_tag_sha256("duplicate_test", sha)
    await add_tag_sha256("duplicate_test", sha)
